...
#define MONITOR_WINDOW_MS 300000
#define BLE_ON_MS 10000
#define RESTING_10_MIN_MS 600000

#define RESTING_INTERVAL_MS 10000
#define WALKING_INTERVAL_MS 1000
#define RUNNING_INTERVAL_MS 500
#define DEEP_SLEEP_INTERVAL_MS 100000
...
void axp2101_init() {
    ...
    i2c_cmd_link_delete(cmd);
    if (ret == ESP_OK) {
        float battery_voltage = (data * 1.1f) / 1000.0f;
...
void axp2101_prepare_deep_sleep() {
    ...
    i2c_cmd_link_delete(cmd);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Deep sleep prep failed: %s", esp_err_to_name(ret));
    } else {
        ESP_LOGI(TAG, "Prepared for Deep Sleep");
    }
}
...
void calibrate_sensor() {
    ...
    ESP_LOGI(TAG, "Calibrating... Keep device stationary.");
    ...
    if (valid_samples > 0) {
        ...
        ESP_LOGI(TAG, "Calibrated: Offset(%.3f, %.3f, %.3f), Scale: %.3f",
                 offset_x, offset_y, offset_z, scale_factor);
    } else {
        ESP_LOGE(TAG, "Calibration failed. Using fallback scale.");
    }
}
                ...
                if (buffer_full) {
                    ...
                    if (strcmp(current_activity, "Resting") == 0 && resting_start_time != 0) {
                        ...
                        if (resting_duration_ms > RESTING_10_MIN_MS) {
                            ...
                            ESP_LOGI(TAG, "Resting > 10 min: Deep Sleep");
