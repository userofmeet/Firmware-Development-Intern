    ...
    while (1) {
        if (qmi.getDataReady()) {
            if (qmi.getAccelerometer(acc.x, acc.y, acc.z)) {
                ...
                // Parameters for smooth decay
                static const float decay_factor = 0.90f;  // adjust between 0.80f (fast drop) to 0.99f (very slow)

                // If resting, decay the speed
                if (strcmp(activity, "Resting") == 0) {
                    speed_mps *= decay_factor;
                    if (speed_mps < 0.01f) speed_mps = 0.0f;  // clamp to zero if too small
                } 
                // Else: detect step & update speed
                else if (detect_step(avg_magnitude)) {
                    step_count++;
